...
int main()
{
    ...
    orPerceptron.train(trainingData, targets, maxEpochs, learningRate);

    // Test the trained perceptron
    cout << "OR Perceptron Output after Training:" << endl;
    for (size_t i = 0; i < trainingData.size(); ++i)
    {
        int prediction = orPerceptron.activate(trainingData[i][0], trainingData[i][1]);
        cout << trainingData[i][0] << " OR " << trainingData[i][1] << " = " << prediction << endl;
    }
